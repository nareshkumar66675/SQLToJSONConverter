...
UPDATE AD
SET AD.AST_OPTN_VALUE = 'DEFAULT'
--select * 
FROM GAM.ASSET_DETAIL AS AD
JOIN GAM.ASSET_STANDARD_DETAILS AS ASD ON ASD.ASD_STD_ID = AD.ASD_STD_ID
WHERE AST_OPTION_ID = 210 AND ASD.IS_DELETED = 0  AND ASD.ASD_CLST_STAT_ID = 5
AND AST_OPTN_VALUE = ''
GO
...
JOIN GAM.MODEL (NOLOCK) AS MDL ON MDL.MDL_ID = ASD.ASD_MODL_ID
...
AND MDL_LONG_NAME NOT IN ('POS') AND MDL.IS_DELETED = 0
...
WHERE MDL_LONG_NAME NOT IN ('POS') AND MDL.IS_DELETED = 0
GO
