...
END AS MNF_CODE_SEQ,
 VALUE 
 FROM
(SELECT MANF_ID, ROW_NUMBER() OVER (ORDER BY MANF_ID ) AS MF_ID,
...
FROM GAM.MANUFACTURER  WHERE IS_DELETED = 0 AND MANF_IS_VENDOR = 0 and MANF_ID > 0) MFN
UNPIVOT 
...
END AS MDL_SEQ,
VALUE
FROM ( SELECT MDL.MDL_ID, MDL_MANF_ID, MDL_SHORT_NAME as Name,
...
FROM  GAM.MODEL AS MDL WHERE MDL.IS_DELETED = 0 ) MDL_TYPE
UNPIVOT
...
FROM (SELECT MTR_ID , m.INSMAP_ID , MTR_NAME , m.PRGP_ID , m.JKPT_ID , 
