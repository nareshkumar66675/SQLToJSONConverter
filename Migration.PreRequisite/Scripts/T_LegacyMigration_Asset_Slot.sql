...
JOIN MIGRATION.GAM_TYPE_DESCRIPTION_WITH_ASSET (nolock) AS ASTMAP ON ASTMAP.TYPEDESCRIPTION = TYP.TYPEDESCRIPTION
...
left join [MIGRATION].[GAM_ASSET_STANDARD_DETAILS] (nolock) on [ASD_STD_LEGACY_ID] = ASD_STD_ID
GO
...
IF EXISTS (SELECT * FROM [SYS].[INDEXES] WHERE [NAME] = 'GAM_ASSET_COMP_VALUES_INDX')
BEGIN
DROP INDEX [GAM_ASSET_COMP_VALUES_INDX] ON [MIGRATION].[GAM_ASSET_COMP_VALUES]
END
...
LEFT JOIN GAM.AREA (nolock) AS AR ON AR.IS_DELETED = 0 AND AR.AREA_ID = ASD.ASD_AREA_ID
left join migration.gam_area (nolock) as gar on gar.[AREA_LEGCY_ID] = ASD.ASD_AREA_ID

WHERE ASD.IS_DELETED = 0 AND ASD_CLST_STAT_ID = 5
AND AST.ASST_ID in ( 1 ) AND MANF_LONG_NAME NOT IN ('POS') 
And ST.SITE_NUMBER in (1, 2, 3, 301, 302, 401, 404, 501, 502, 201, 202, 801, 802, 683, 601)
...
JOIN GAM.MODEL (nolock) AS MDL ON MDL.MDL_ID = ASD_MODL_ID
JOIN GAM.MANUFACTURER (nolock) AS MF ON MF.MANF_ID = ASD_MANF_ID
LEFT JOIN GAM.ZONE (nolock) AS ZN ON ZN.IS_DELETED = 0 AND ZN.ZONE_ID = ASD.ASD_ZONE_ID
left join migration.gam_zone (nolock) as gzn on gzn.[ZONE_LEGCY_ID] = ASD.ASD_ZONE_ID
left join migration.GAM_AREA (nolock) as gar on gar.[AREA_LEGCY_ID] = ASD.ASD_AREA_ID
WHERE ASD.IS_DELETED = 0 AND ASD_CLST_STAT_ID = 5
AND AST.ASST_ID in ( 1 ) AND MANF_LONG_NAME NOT IN ('POS') 
And ST.SITE_NUMBER in (1, 2, 3, 301, 302, 401, 404, 501, 502, 201, 202, 801, 802, 683, 601)
...
JOIN GAM.MODEL (nolock) AS MDL ON MDL.MDL_ID = ASD_MODL_ID
JOIN GAM.MANUFACTURER (nolock) AS MF ON MF.MANF_ID = ASD_MANF_ID
LEFT JOIN GAM.BANK (nolock) AS BK ON bk.IS_DELETED = 0 AND BK.BANK_ID = ASD.ASD_BANK_ID
left join migration.gam_bank (nolock) as gbk on gbk.[BANK_LEGCY_ID] = ASD.ASD_BANK_ID
left join migration.GAM_AREA (nolock) as gar on gar.[AREA_LEGCY_ID] = ASD.ASD_AREA_ID
WHERE ASD.IS_DELETED = 0 AND ASD_CLST_STAT_ID = 5
AND AST.ASST_ID in ( 1 ) AND MANF_LONG_NAME NOT IN ('POS')
And ST.SITE_NUMBER in (1, 2, 3, 301, 302, 401, 404, 501, 502, 201, 202, 801, 802, 683, 601)
...
LEFT JOIN [MIGRATION].[GAM_MANUFACTURER] (nolock) AS GMNF ON MNF.MANF_ID = GMNF.MNF_LEGCY_ID
...
JOIN GAM.MANUFACTURER (nolock) AS MF ON MF.MANF_ID = ASD_MANF_ID
JOIN GAM.MODEL (nolock) AS MDL ON MDL.MDL_ID = ASD_MODL_ID
JOIN GAM.ASSET_DETAIL (nolock) AS AD ON  AD.IS_DELETED = 0 AND ad.[AST_OPTION_ID] = 126 and AD.ASD_STD_ID = ASD.ASD_STD_ID
LEFT JOIN [GAM].MODEL_TYPE (nolock) AS MTYP ON  MTYP.IS_DELETED = 0 AND  MDLTYP_LONG_NAME = AST_OPTN_VALUE
left join [MIGRATION].[GAM_MODEL_TYPE] (nolock) as gmty on gmty.MDLTYPE_LEGCY_ID = [MDLTYP_ID]
LEFT JOIN MIGRATION.GAM_TYPE_DESCRIPTION_WITH_ASSET (nolock) AS GTC ON GTC.ASD_STD_ID = ASD.ASD_STD_ID 
LEFT JOIN MIGRATION.GAM_TYPE_DESCRIPTION (nolock) AS tc on tc.Id = gtc.typedescp_id 
WHERE ASD.IS_DELETED = 0 AND ASD_CLST_STAT_ID = 5
AND AST.ASST_ID in ( 1 ) AND MANF_LONG_NAME NOT IN ('POS')
...
join [MIGRATION].[GAM_PROPERTY] (nolock) as gpt on gpt.[PROP_LEGCY_ID] = pt.PROP_ID
JOIN GAM.MANUFACTURER (nolock) AS MF ON MF.MANF_ID = ASD_MANF_ID
JOIN GAM.ASSET_DETAIL (nolock) AS AD ON AD.IS_DELETED = 0 AND  ad.[AST_OPTION_ID] = 108 and AD.ASD_STD_ID = ASD.ASD_STD_ID
LEFT JOIN [GAM].MODEL (nolock) AS MDL ON MDL.IS_DELETED = 0 AND [MDL_LONG_NAME] = AST_OPTN_VALUE
LEFT JOIN MIGRATION.GAM_MODEL (nolock) AS GDML ON MDL.MDL_ID = GDML.MDL_LEGCY_ID --MDL_NEW_ID
LEFT JOIN MIGRATION.GAM_TYPE_DESCRIPTION_WITH_ASSET (nolock) AS GTC ON GTC.ASD_STD_ID = ASD.ASD_STD_ID 
LEFT JOIN MIGRATION.GAM_TYPE_DESCRIPTION (nolock) AS tc on tc.Id = gtc.typedescp_id 
WHERE ASD.IS_DELETED = 0 AND ASD_CLST_STAT_ID = 5
AND AST.ASST_ID in ( 1 ) AND MANF_LONG_NAME NOT IN ('POS') 
...
JOIN GAM.MODEL (nolock) AS MDL ON MDL.MDL_ID = ASD_MODL_ID
JOIN GAM.MANUFACTURER (nolock) AS MF ON MF.MANF_ID = ASD_MANF_ID
LEFT JOIN GAM.ASSET_DETAIL (nolock) AS AD ON  AD.IS_DELETED = 0 AND ad.[AST_OPTION_ID] = 202 and AD.ASD_STD_ID = ASD.ASD_STD_ID
LEFT JOIN MIGRATION.GAM_DENOMINATION (nolock) AS GD ON GD.DENM_AMOUNT = AST_OPTN_VALUE
WHERE ASD.IS_DELETED = 0 AND ASD_CLST_STAT_ID = 5
AND AST.ASST_ID in ( 1 ) AND MANF_LONG_NAME NOT IN ('POS') 
And ST.SITE_NUMBER in (1, 2, 3, 301, 302, 401, 404, 501, 502, 201, 202, 801, 802, 683, 601)
GO
...
JOIN GAM.MANUFACTURER (nolock) AS MF ON MF.MANF_ID = ASD_MANF_ID
JOIN GAM.MODEL (nolock) AS MDL ON MDL.MDL_ID = ASD_MODL_ID
LEFT JOIN GAM.ASSET_DETAIL (nolock) AS AD ON  AD.IS_DELETED = 0 AND ad.[AST_OPTION_ID] = 203 and AD.ASD_STD_ID = ASD.ASD_STD_ID
LEFT JOIN MIGRATION.GAM_DENOMINATION (nolock) AS GD ON GD.DENM_AMOUNT = AST_OPTN_VALUE
WHERE ASD.IS_DELETED = 0 AND ASD_CLST_STAT_ID = 5
AND AST.ASST_ID in ( 1 ) AND MANF_LONG_NAME NOT IN ('POS') 
And ST.SITE_NUMBER in (1, 2, 3, 301, 302, 401, 404, 501, 502, 201, 202, 801, 802, 683, 601)
...
JOIN GAM.MANUFACTURER (nolock) AS MF ON MF.MANF_ID = ASD_MANF_ID
JOIN GAM.MODEL (nolock) AS MDL ON MDL.MDL_ID = ASD_MODL_ID
LEFT JOIN GAM.ASSET_DETAIL (nolock) AS AD ON AD.IS_DELETED = 0 AND ad.[AST_OPTION_ID] = 204 and AD.ASD_STD_ID = ASD.ASD_STD_ID
LEFT JOIN MIGRATION.GAM_DENOMINATION (nolock) AS GDM ON GDM.DENM_AMOUNT = AD.AST_OPTN_VALUE
WHERE ASD.IS_DELETED = 0 AND ASD_CLST_STAT_ID = 5
AND AST.ASST_ID in ( 1 ) AND MANF_LONG_NAME NOT IN ('POS') 
And ST.SITE_NUMBER in (1, 2, 3, 301, 302, 401, 404, 501, 502, 201, 202, 801, 802, 683, 601)
GO
...
JOIN GAM.MANUFACTURER (nolock) AS MF ON MF.MANF_ID = ASD_MANF_ID
JOIN GAM.MODEL (nolock) AS MDL ON MDL.MDL_ID = ASD_MODL_ID
LEFT JOIN GAM.ASSET_DETAIL (nolock) AS AD ON  AD.IS_DELETED = 0 AND ad.[AST_OPTION_ID] = 210 and AD.ASD_STD_ID = ASD.ASD_STD_ID
LEFT JOIN MIGRATION.GAM_TYPE_DESCRIPTION_WITH_ASSET (nolock) AS TYAST ON TYAST.ASD_STD_ID = ASD.ASD_STD_ID
LEFT JOIN MIGRATION.GAM_TYPE_DESCRIPTION (nolock) AS TDSC ON TDSC.ID = TYAST.TYPEDESCP_ID
WHERE ASD.IS_DELETED = 0 AND ASD_CLST_STAT_ID = 5
AND AST.ASST_ID in ( 1 ) AND MANF_LONG_NAME NOT IN ('POS') 
And ST.SITE_NUMBER in (1, 2, 3, 301, 302, 401, 404, 501, 502, 201, 202, 801, 802, 683, 601)
GO
...
JOIN GAM.MANUFACTURER (nolock) AS MF ON MF.MANF_ID = ASD_MANF_ID
JOIN GAM.MODEL (nolock) AS MDL ON MDL.MDL_ID = ASD_MODL_ID
LEFT JOIN GAM.ASSET_DETAIL (nolock) AS AD ON  AD.IS_DELETED = 0 AND ad.[AST_OPTION_ID] = 206 and AD.ASD_STD_ID = ASD.ASD_STD_ID
LEFT JOIN MIGRATION.GAM_TYPE_DESCRIPTION_WITH_ASSET (nolock) AS GTC ON GTC.ASD_STD_ID = ASD.ASD_STD_ID 
LEFT JOIN MIGRATION.GAM_TYPE_DESCRIPTION (nolock) AS tc on tc.Id = gtc.typedescp_id 
WHERE ASD.IS_DELETED = 0 AND ASD_CLST_STAT_ID = 5
AND AST.ASST_ID in ( 1 ) AND MANF_LONG_NAME NOT IN ('POS') 
...
JOIN GAM.MANUFACTURER (nolock) AS MF ON MF.MANF_ID = ASD_MANF_ID
JOIN GAM.MODEL (nolock) AS MDL ON MDL.MDL_ID = ASD_MODL_ID
LEFT JOIN GAM.ASSET_DETAIL (nolock) AS AD ON  AD.IS_DELETED = 0 AND ad.[AST_OPTION_ID] = 207 and AD.ASD_STD_ID = ASD.ASD_STD_ID
LEFT JOIN MIGRATION.GAM_TYPE_DESCRIPTION_WITH_ASSET (nolock) AS GTC ON GTC.ASD_STD_ID = ASD.ASD_STD_ID 
LEFT JOIN MIGRATION.GAM_TYPE_DESCRIPTION (nolock) AS tc on tc.Id = gtc.typedescp_id 
WHERE ASD.IS_DELETED = 0 AND ASD_CLST_STAT_ID = 5
AND AST.ASST_ID in ( 1 ) AND MANF_LONG_NAME NOT IN ('POS') 
...
JOIN GAM.MODEL (nolock) AS MDL ON MDL.MDL_ID = ASD_MODL_ID
JOIN GAM.MANUFACTURER (nolock) AS MF ON MF.MANF_ID = ASD_MANF_ID
LEFT JOIN GAM.ASSET_DETAIL (nolock) AS AD ON  AD.IS_DELETED = 0 AND ad.[AST_OPTION_ID] = 244 and AD.ASD_STD_ID = ASD.ASD_STD_ID
LEFT JOIN MIGRATION.GAM_TYPE_DESCRIPTION_WITH_ASSET (nolock) AS GTC ON GTC.ASD_STD_ID = ASD.ASD_STD_ID 
LEFT JOIN MIGRATION.GAM_TYPE_DESCRIPTION (nolock) AS tc on tc.Id = gtc.typedescp_id 
WHERE ASD.IS_DELETED = 0 AND ASD_CLST_STAT_ID = 5
AND AST.ASST_ID in ( 1 ) AND MANF_LONG_NAME NOT IN ('POS') 
...
JOIN GAM.MANUFACTURER (nolock) AS MF ON MF.MANF_ID = ASD_MANF_ID
JOIN GAM.MODEL (nolock) AS MDL ON MDL.MDL_ID = ASD_MODL_ID
LEFT JOIN GAM.ASSET_DETAIL (nolock) AS AD ON  AD.IS_DELETED = 0 AND ad.[AST_OPTION_ID] = 246 and AD.ASD_STD_ID = ASD.ASD_STD_ID
LEFT JOIN MIGRATION.GAM_TYPE_DESCRIPTION_WITH_ASSET (nolock) AS GTC ON GTC.ASD_STD_ID = ASD.ASD_STD_ID 
LEFT JOIN MIGRATION.GAM_TYPE_DESCRIPTION (nolock) AS tc on tc.Id = gtc.typedescp_id 
WHERE ASD.IS_DELETED = 0 AND ASD_CLST_STAT_ID = 5
AND AST.ASST_ID in ( 1 ) AND MANF_LONG_NAME NOT IN ('POS')
...
JOIN GAM.MODEL (nolock) AS MDL ON MDL.MDL_ID = ASD_MODL_ID
JOIN GAM.MANUFACTURER (nolock) AS MF ON MF.MANF_ID = ASD_MANF_ID
LEFT JOIN GAM.ASSET_DETAIL (nolock) AS AD ON  AD.IS_DELETED = 0 AND ad.[AST_OPTION_ID] = 247 and AD.ASD_STD_ID = ASD.ASD_STD_ID
LEFT JOIN MIGRATION.GAM_TYPE_DESCRIPTION_WITH_ASSET (nolock) AS GTC ON GTC.ASD_STD_ID = ASD.ASD_STD_ID 
LEFT JOIN MIGRATION.GAM_TYPE_DESCRIPTION (nolock) AS tc on tc.Id = gtc.typedescp_id 
WHERE ASD.IS_DELETED = 0 AND ASD_CLST_STAT_ID = 5
AND AST.ASST_ID in ( 1 ) AND MANF_LONG_NAME NOT IN ('POS')
...
IF EXISTS(select * FROM sys.views where name = 'VIEW_ASST_SUMMARY')
DROP VIEW  MIGRATION.VIEW_ASST_SUMMARY
...
COUNT(ASD.ASD_STD_ID) AS ASSET_COUNT FROM GAM.ASSET_STANDARD_DETAILS (nolock) AS ASD
JOIN MIGRATION.GAM_ASSET_STANDARD_DETAILS (nolock) AS GSD ON ASD.ASD_STD_ID = GSD.ASD_STD_LEGACY_ID
JOIN GAM.ASSET (nolock) AS AST ON AST.ASST_ID = ASD.ASD_ASST_ID
JOIN GAM.INSTALLED_SYSTEM_MAP (nolock) AS MAP ON MAP.INSM_ID = ASD.ASD_INSMAP_ID
JOIN GAM.[SITE] (nolock) AS ST ON ST.SITE_ID = MAP.INSM_SITE_ID
