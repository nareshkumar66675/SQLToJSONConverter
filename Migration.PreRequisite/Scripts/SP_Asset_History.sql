...
LEFT JOIN GAM.CHANGE_LIST_STATUS (nolock) AS STS ON CL.CLST_STAT_ID = STS.CLST_STAT_ID
...
LEFT JOIN MIGRATION.GAM_CHANGELIST_ASSET_DETAIL (nolock) AS MCD ON MCD.CLST_DET_ID = CAD.CLST_DET_ID
...
CASE WHEN OPTN_CODE in ('ASSET.GMU.CRC.AUTH', 'ASSET.CASLESS.DISAB', 'OPTION.CODE.ENROLMENT.STATUS') THEN 
	(CASE	WHEN CURRENT_VALUE = 'FLAG.NO' then 'N'
		...
		WHEN CURRENT_VALUE = 'FLAG.YES' then 'Y' END)
 ELSE 
...
CASE WHEN OPTN_CODE in ('ASSET.GMU.CRC.AUTH', 'ASSET.CASLESS.DISAB', 'OPTION.CODE.ENROLMENT.STATUS') THEN 
	(CASE	WHEN OLD_VALUE = 'FLAG.NO' then 'N'
		...
		WHEN OLD_VALUE = 'FLAG.YES' then 'Y' END)
 ELSE 
...
SET @TMP = '{ "'
SELECT @TMP = @TMP + 
CASE WHEN OPTN.OPTN_CODE = 'ASSET.MASTER.MANUFACTURER' THEN 'MANUFACTURER' ELSE OPTN.OPTN_CODE END +'":"'+

CASE WHEN OPTN.OPTN_CODE in ('ASSET.GMU.CRC.AUTH', 'ASSET.CASLESS.DISAB', 'OPTION.CODE.ENROLMENT.STATUS') THEN 
 ...
 ELSE
	(CASE WHEN AST_OPTN_VALUE = 'FLAG.NO' then 'No'
      ...
      ELSE ISNULL(cast(AST_OPTN_VALUE as nvarchar), '') END)  END
