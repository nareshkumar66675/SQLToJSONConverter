...
SELECT ROW_NUMBER() over (partition by gdm_aststd_or_tycd_id order by gdm_aststd_or_tycd_id, Game_id, Id)  as Idx,
GDM_ASTSTD_OR_TYCD_ID as GM_ASD_STD_ID,
gm.Game_Id, ASD_AM_UID, gmOptionName, Code, Id, GmOptionValue, Comp_Id
...
SET @TMP = '{ "'
SELECT @TMP = @TMP + OPTN.OPTN_CODE +'":"'+ ISNULL(AST_OPTN_VALUE, '') + '", "' FROM GAM.ASSET_DETAIL AS AD
