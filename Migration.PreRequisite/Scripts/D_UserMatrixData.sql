...
/* Get Site Specific Data  - Contact Information  */
...
COUNTRY_IS_SUPPORTED) VALUES ('+
	   ...
	   ISNULL(CAST(CONVERT(DATETIME,ORG_CREATED_TS,1)AS VARCHAR(100)),'')+''','''+
	   ISNULL(CAST(CONVERT(DATETIME,ORG_UPDATED_TS,1)AS VARCHAR(100)),'')+''','''+
	   ...
	   ');' as Script 
	    FROM [UM].[ORGANIZATION](nolock) AS ORG
INNER JOIN [UM].[CONTACT_MASTER](nolock) AS C ON C.CONT_ID = ORG_CONT_ID
LEFT JOIN [UM].[COUNTRY](nolock) as cnt on cnt.COUNTRY_CODE=c.CONT_COUNTRY_CODE
